...
export default function ProposalsPage() {
    ...
    const handleGenerate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            else alert('Erro ao criar proposta: ' + (result.error || 'Erro desconhecido'))
        } catch (error) { console.error(error); alert('Erro ao criar proposta') } finally { setLoading(false); setUploading(false) }
    }
    ...
    return (
        <DashboardLayout>
            <header className="mb-10">
                <h1 className="text-3xl font-bold mb-2">Gerador de <span className="accent-text">Propostas</span></h1>
                <p className="text-text-muted">Crie orçamentos profissionais em segundos com auxílio de IA.</p>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="glass-card p-10 bg-white border-none shadow-sm h-fit">
                    ...
                    <form className="space-y-6" onSubmit={handleGenerate}>
                        ...
                        <div className="space-y-2">
                            <Label className="font-bold text-navy uppercase text-[10px] tracking-widest">Imagem / Croqui</Label>
                            {!selectedFile ? (
                                <div className="border-2 border-dashed border-border rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer hover:bg-bg-dark transition-colors" onClick={() => fileInputRef.current?.click()}>
                                    ...
                                    <span className="text-sm font-bold text-navy">Subir Imagem</span>
                        ...
                        <Button type="submit" className="w-full py-4 bg-accent text-navy font-black rounded-xl hover:shadow-xl transition-all h-auto text-lg" disabled={loading}>
                            ...
                            {loading ? 'Processando...' : 'GERAR PROPOSTA COM IA'}
                        </Button>
                    </form>
                </div>
